...
from ._utils_metadata import _convert_datetime_column
...
class ColumnPairTrends(BaseSingleTableProperty):
    ...
    def _get_columns_data_and_metric(
        ...
        if self._sdtype_to_shape[sdtype_col_1] != self._sdtype_to_shape[sdtype_col_2]:
            ...
            if self._sdtype_to_shape[sdtype_col_1] == 'continuous':
                ...
                self._discretized_pairs.append((column_name_1, column_name_2))
            else:
    ...
    def _generate_details(
        ...
        for column_names in column_pairs:
            ...
            try:
                ...
                if metric.__name__ == 'CorrelationSimilarity':
                    ...
                    # Manually compute Cramer's V for categorical pairs in the real data
                    try:
                        # col_real is a DataFrame with two columns
                        x = col_real.iloc[:, 0]
                        y = col_real.iloc[:, 1]
                        # build contingency table
                        ct = pd.crosstab(x, y)
                        # need at least 2 rows and 2 cols to compute Cramer's V
                        if ct.shape[0] < 2 or ct.shape[1] < 2:
                            cramers_v = np.nan
                        else:
                            chi2, pval, dof, expected = chi2_contingency(ct, correction=False)
                            n = ct.values.sum()
            ...
            discretized = []
            for idx in range(len(column_names_1)):
                if (column_names_1[idx], column_names_2[idx]) in self._discretized_pairs:
                    discretized.append(True)
                else:
                    discretized.append(False)
        ...
        print(f"Discretized pairs: {self._discretized_pairs}")
